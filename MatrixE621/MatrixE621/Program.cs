...
namespace MpdDj
{
	class Program
	{
		...
		public static void Main (string[] args)
		{
			...
			string cfgpath;
			http = new HttpClient();
			http.DefaultRequestHeaders.UserAgent.ParseAdd("MatrixE621");

			if (args.Length > 1) {
				cfgpath = args [1];
			} else {
				cfgpath = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile) + "/.config/matrix_e621.ini";
			}
